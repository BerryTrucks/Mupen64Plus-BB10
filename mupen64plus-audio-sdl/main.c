...
EXPORT void CALL RomClosed( void )
{
   ...
   if (critical_failure == 1)
       return;
    DebugMessage(M64MSG_INFO, "Cleaning up SDL sound plugin...");
    ...
    DebugMessage(M64MSG_INFO, "Primary Buffer freed...");
    if (mixBuffer != NULL)
    {
        free(mixBuffer);
        mixBuffer = NULL;
    }
    DebugMessage(M64MSG_INFO, "Mix Buffer freed...");

    // Delete the hardware spec struct
    if(hardware_spec != NULL) free(hardware_spec);
    hardware_spec = NULL;

    DebugMessage(M64MSG_INFO, "HW Spec Buffer freed...");
    ...
    DebugMessage(M64MSG_INFO, "ROM CLosed...");
}
